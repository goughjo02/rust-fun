...
/// to leave the circle of radius 2 centered on the origin. If `c` seems to be a member (more precisely,
...
/// 
/// The `bounds` argument gives the width and height of the buffer `pixels`, which holds one grayscale
/// pixel per byte. The `upper_left` and `lower_right` arguments specify points on the complex plane
